"""Reselect real labels using optimized metadata camera/LiDAR extrinsics.

This is an isolated alternative to ``select_real_label_candidates.py`` for an
extrinsic produced by ``optimize_camera_lidar_extrinsics.py`` with
``--use-sample-metadata``.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Select GigaPose/EPnP pairs using one optimized fixed "
            "camera/LiDAR transform for the calibrated camera."
        )
    )
    parser.add_argument(
        "--source-root",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "It must come from optimize_camera_lidar_extrinsics "
            "--use-sample-metadata mode."
        ),
    )
    parser.add_argument(
...
def load_optimized_extrinsic(path: Path) -> tuple[np.ndarray, dict[str, Any], Path]:
    resolved = resolve_extrinsics_path(path)
    data = json.loads(resolved.read_text())
    mode = str(data.get("optimization_mode", ""))
    key = "T_lidar_camera_optimized"
    value = data.get(key)
    if value is None:
        raise ValueError(
            f"{resolved} does not contain {key}. Its optimization_mode is "
            f"{mode!r}; rerun optimize_camera_lidar_extrinsics with "
            "--use-sample-metadata."
        )
    expected_mode = "fixed_lidar_camera_from_robust_metadata_initial"
    if mode != expected_mode:
        raise ValueError(
            f"{resolved} has optimization_mode={mode!r}, but this selector "
            f"requires {expected_mode!r}."
        )
    optimized = np.asarray(value, dtype=float).reshape(4, 4)
    rigid_transform_checks(optimized, key)
    return optimized, data, resolved
        ...
        for path in sorted(root.glob(glob_pattern)):
            ...
            for record_index, record in enumerate(records):
                try:
                    ...
                    metadata_row = {"epnp_label_path": str(path)}
                    ...
                    T_lidar_camera_prior = matrix_from_yaml_key(
                        metadata, "t_lidar_camera_prior", "m"
                    )
                    rigid_transform_checks(T_map_lidar, "t_map_lidar")
                    rigid_transform_checks(
                    ...
                    skipped["missing_or_malformed_metadata"] += 1
                    if skipped["missing_or_malformed_metadata"] <= 5:
                        print(f"Skipping label without usable metadata {path}: {exc}")
                    continue
                except Exception as exc:
...
def make_candidate_rows(
    ...
    for key in shared_keys:
        ...
        for label in labels:
            ...
            T_map_camera_initial = T_map_lidar @ T_lidar_camera_prior
            ...
            T_camera_object_optimized = (
                T_camera_map_optimized @ T_map_object
            )
                ...
                rows.append(
                    {
                        ...
                        "T_map_cam_label_derived": base.matrix_to_text(
                            T_map_camera_label_derived
                        ),
...
def main() -> None:
    ...
    T_lidar_camera_optimized, extrinsics_data, extrinsics_path = load_optimized_extrinsic(
    ...
    calibrated_camera = str(extrinsics_data.get("calibrated_camera") or "")
    loaded_cameras = sorted({str(label["metadata_camera"]) for label in labels})
    if not calibrated_camera:
        raise ValueError(
            f"{extrinsics_path} does not identify calibrated_camera; rerun "
            "fine_tuning.optimize_camera_lidar_extrinsics first."
        )
    if loaded_cameras != [calibrated_camera]:
        raise ValueError(
            "The optimized calibration cannot be reused across camera types: "
            f"extrinsics are for {calibrated_camera!r}, labels contain "
            f"{loaded_cameras}."
        )
    ...
    report: dict[str, Any] = {
        "description": (
            ...
            "object-frame transform as select_real_label_candidates. For each "
            "sample it composes the camera's fixed optimized "
            "T_lidar_camera with that frame's t_map_lidar and compares "
            "GigaPose with the resulting corrected EPnP camera-frame pose."
        ),
