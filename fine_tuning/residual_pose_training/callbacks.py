"""Metric history callback for residual-pose experiments."""
...
class ResidualMetricHistoryCallback(pl.Callback):
    """Write every residual loss and human-readable monitor to CSV."""
    ...
    def _write(self, trainer: pl.Trainer, stage: str, prefix: str) -> None:
        ...
        for name, value in sorted(trainer.callback_metrics.items()):
            ...
            if not (
                short_name.startswith("loss")
                or short_name.startswith("monitor_")
                or short_name in {"valid_instances", "valid_patch_pairs"}
            ):
                continue
