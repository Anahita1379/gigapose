"""Export sequence-safe GigaPose/RGB-CAD candidate sets with oracle labels."""
...
from .checkpoints import load_candidate_predictor
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--translation-label-scale-m", type=float, default=1.0)
    value.add_argument("--rotation-label-scale-deg", type=float, default=20.0)
...
def _validate(args) -> None:
    if args.saved_candidates < 2:
        raise ValueError("--saved-candidates must include baseline plus at least one recovery")
    if args.max_candidates < args.saved_candidates - 1:
        raise ValueError("--max-candidates must cover the requested recovered candidates")
    if args.translation_label_scale_m <= 0 or args.rotation_label_scale_deg <= 0:
        raise ValueError("Oracle label scales must be positive")
    if args.sequence_max_frame_gap < 0 or args.sequence_max_time_gap_s < 0:
        raise ValueError("Sequence gap thresholds must be non-negative")
...
def _mesh(args) -> Path:
    path = args.mesh or args.dataset_dir / "models" / "obj_000001.ply"
    if not path.is_file():
        raise FileNotFoundError(f"Candidate CAD is absent: {path}")
    return path


def _score(predictor, renderer, frame, detection, proposals, args, iterations):
    return predictor.refine_and_score(
        renderer,
        frame,
        detection,
        proposals,
...
def main() -> None:
    ...
    try:
        for frame_index, frame in enumerate(source):
            ...
            if gt_pose is None or len(frame.detections) != 1:
                skipped.append({"scene_id": key[0], "im_id": key[1], "reason": "requires one GT and one detection"})
                continue
            ...
            if not fresh:
                skipped.append({"scene_id": key[0], "im_id": key[1], "reason": "no assigned GigaPose hypotheses"})
                continue

            baseline_pose = fresh[0].pose.copy()
            baseline_proposal = RecoveryProposal(
                baseline_pose, "gigapose_rank0_baseline", fresh[0].measurement_score, 0.0
            )
            baseline_scored = _score(
                predictor, renderer, frame, detection, [baseline_proposal], args, 0
            )
            if not baseline_scored:
                skipped.append({"scene_id": key[0], "im_id": key[1], "reason": "baseline scoring failed"})
                continue
            ...
            selected = [baseline_scored[0], *recovered[: args.saved_candidates - 1]]
            valid = np.zeros(args.saved_candidates, dtype=bool)
            valid[: len(selected)] = True
            poses = np.repeat(np.eye(4, dtype=np.float32)[None], args.saved_candidates, axis=0)
            features = np.zeros(
            ...
            for index, result in enumerate(selected):
                poses[index] = np.asarray(result.pose, dtype=np.float32)
                features[index] = candidate_features(
                    result, baseline_pose, baseline=index == 0
                )
                sources[index] = result.source
            cost, translation, rotation = oracle_costs(
                poses,
                gt_pose,
                valid,
                translation_scale_m=args.translation_label_scale_m,
                rotation_scale_deg=args.rotation_label_scale_deg,
            )
            # Segment against the previous *saved* sample. A frame omitted above
            # therefore becomes a real temporal discontinuity in the GRU data.
            ...
            if previous_saved_stamp is None or reason is not None:
                ...
                delta_time = (
                    0.0
                    if previous_saved_time is None or stamp.time_s is None
            ...
            records.append(
                {
                    ...
                    "valid": valid,
                    "sources": sources,
                    "ground_truth_pose": gt_pose.astype(np.float32),
                    "oracle_cost": cost,
                    "translation_error_m": translation,
                    "rotation_error_deg": rotation,
                    "oracle_index": int(np.argmin(cost)),
                }
            )
    ...
    if not records:
        raise RuntimeError("No candidate frames were exported")
    names = (
        "scene_id", "im_id", "source_frame", "time_s", "segment_id", "poses",
        "features", "frame_features", "valid", "sources", "ground_truth_pose",
        "oracle_cost", "translation_error_m", "rotation_error_deg", "oracle_index",
    )
    arrays = {name: np.asarray([row[name] for row in records]) for name in names}
    arrays["sequence_run"] = np.asarray([row["sequence_run"] for row in records], dtype="<U256")
    arrays["sequence_camera"] = np.asarray([row["sequence_camera"] for row in records], dtype="<U64")
    ...
    report = {
        ...
        "oracle_nonbaseline_fraction": float(np.mean(arrays["oracle_index"] != 0)),
